...
handler: async (ctx,args)=>{
 const identity =await ctx.auth.getUserIdentity();
  ...
  const receiver= await ctx.db.query("users").withIndex("by_email",q=>
  ...
  .withIndex("by_receiver_sender",q=>q.eq
    ("receiver",receiver._id).eq("sender",currentUser._id) 
  ).unique()
  ...
  .withIndex("by_receiver_sender",q=>q.eq
    ("receiver",currentUser._id).eq("sender",receiver._id) 
  ).unique()
  ...
  if(friends1.some(friend =>friend.user2 === receiver._id)|| friends2.some(friend=>
